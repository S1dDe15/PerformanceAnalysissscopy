namespace PerformanceAnalysis.Infrastructure.Reports
{
    public class ReportQueries
    {
        ...
        WITH BestAttempt AS (
            ...
            WHERE tr.studentid = @a.studentid
        ...
        GROUP BY g.id, g.name, DATE_TRUNC('month', a.startedat),
        ...
        public const string StudentPassRate = @"
            SELECT s.id AS studentid,
                   u.firstname || ' ' || u.lastname AS fullname,
                   g.name AS group,
                   COUNT(DISTINCT t.id) AS testavailable,
                   COUNT(DISTINCT CASE WHEN tr.passed = TRUE THEN t.id END) AS testpassed,
